  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {[
          { label: 'Overall Progress', value: `${Math.round(studyPlan.reduce((acc, w) => acc + w.completed, 0) / studyPlan.reduce((acc, w) => acc + w.tasks, 0) * 100) || 0}%`, icon: TrendingUp, iconBg: 'bg-indigo-100', iconText: 'text-indigo-600' },
          { label: 'Weeks Completed', value: `${studyPlan.filter(w => w.completed >= w.tasks).length}/4`, icon: CheckCircle2, iconBg: 'bg-green-100', iconText: 'text-green-600' },
          { label: 'Tasks Done', value: `${studyPlan.reduce((acc, w) => acc + w.completed, 0)}`, icon: Target, iconBg: 'bg-amber-100', iconText: 'text-amber-600' },
          { label: 'Study Hours', value: `${stats?.totalStudyHours?.toFixed(1) || '0'}h`, icon: Clock, iconBg: 'bg-purple-100', iconText: 'text-purple-600' },
        ].map((stat) => (
